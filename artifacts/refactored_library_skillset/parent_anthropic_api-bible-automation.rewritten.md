<!-- refactored skeleton for anthropic_api-bible-automation (2 of 10 units replaced by child invocations (cleanup applied)) -->

# API Bible Automation via Rube MCP
invoke(composio-automation, {toolkit_name="API Bible toolkit"})

## Prerequisites
invoke(rube-mcp-connection, {toolkit="api_bible"})  (parent-specific: Always call `RUBE_SEARCH_TOOLS` first to get current tool schemas)
