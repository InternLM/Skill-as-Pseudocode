<!-- refactored skeleton for anthropic_ac-branch-pi-model (1 of 13 units replaced by child invocations (cleanup applied)) -->
...
## Quick start
invoke(power-flow-computation, {branch_data="branches = np.array(data[\"branch\"], dtype=float)", bus_data="buses = np.array(data[\"bus\"], dtype=float)"})  (parent-specific: Use `scripts/branch_flows.py` to compute per-unit branch flows.
- Treat the results as **power leaving** the “from” bus…)
