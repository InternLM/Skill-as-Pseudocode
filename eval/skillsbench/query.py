...
def _is_cos_child(skill: dict[str, Any]) -> bool:
    """True iff this skill is a SaP-refactored child contract (designed to
    be reached via a parent's invoke(), not selected at top-level). The
    marker `_sap_role: child` is written into the SKILL.md frontmatter by
    build_refactored_skillset.py."""
    raw = skill.get("raw_content") or ""
    # Cheap substring check on frontmatter region; avoids parsing YAML.
    head = raw[:400]
    return "_sap_role: child" in head
...
def retrieve(
    ...
    *,
    ...
    all_skills = bundle.get("skills", [])
    edges = bundle.get("edges", [])
    metadata = bundle.get("metadata", {})

    # Filter out SaP child contracts from the top-level retrieval pool.
    # Children are sub-procedures designed to be reached via the parent
    # skill's invoke() placeholders (Fix C / hierarchical-only retrieval).
    skills = [s for s in all_skills if not _is_cos_child(s)]
